...
namespace RelaxingKompas
{
    public partial class FormWeightAndSize : Form
    {
        ...
        private void FormattingText()
        {
            ...
            */
            string plainText = $"{tb_pos.Text}\t\t\t{DataWeightAndSize.Thickness}\t{tb_width.Text}\t{tb_length.Text}\t{tb_weight.Text}\t{tb_steel.Text}\t" +
                $"\t{tb_sheet.Text}\t{tb_yardage.Text}";
            string htmlText = $"<table><tr><td>{tb_pos.Text}</td><td></td><td></td><td>{DataWeightAndSize.Thickness}</td><td>{tb_width.Text}</td>" +
        ...
        private void cb_savefragment_CheckedChanged(object sender, EventArgs e)
        {
            ...
            Properties.Settings.Default.IsCreatFragment= DataWeightAndSize.WindowLibrarySettings.cb_CreatFragment.Checked;
